...
sortBed -i ${filename}_CpG_100bp.bed > ${filename}_CpG_100bp.sorted.bed
sortBed -i ${filename}_CHG_100bp.bed > ${filename}_CHG_100bp.sorted.bed
sortBed -i ${filename}_CHH_100bp.bed > ${filename}_CHH_100bp.sorted.bed

#get total number of columns for both input files
l1="$(cat ${filename}_CpG_100bp.sorted.bed | awk 'BEGIN{FS="\t"};{print NF}' | head -n 1)"
...
closestBed -D "ref" -a ${filename}_CHG_100bp.sorted.bed -b $bedpath > ${filename}_CHG_${outname}.bed

echo "Performing closestBed of CHH methylation..."
closestBed -D "ref" -a ${filename}_CHH_100bp.sorted.bed -b $bedpath > ${filename}_CHH_${outname}.bed

echo "Performing closestBed of CpG methylation..."
closestBed -D "ref" -a ${filename}_CpG_100bp.sorted.bed -b $bedpath > ${filename}_CpG_${outname}.bed
...
#remove sorted bed files
rm ${filename}_CpG_100bp.sorted.bed
rm ${filename}_CHG_100bp.sorted.bed
rm ${filename}_CHH_100bp.sorted.bed
